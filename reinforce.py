...
def return_summary_index(probs_numpy, probs_torch, sample_method="greedy", max_num_of_sents=3):
    ...
    """
    #print("max_num_of_sents =", max_num_of_sents)
    if max_num_of_sents <= 0:
        ...
        if sample_method == "sample":
            ...
            #print("probs_torch:", probs_torch)
            #print("probs_torch.size():", probs_torch.size())
...
class ReinforceReward():
    ...
    def train(self, probs, doc, max_num_of_sents=3, max_num_of_chars=-1, prt=False):
        """
        ...
        batch_rewards = [
            self.generate_reward(idx_list[0], max_num_of_chars)
        ...
        greedy_reward = self.generate_reward(greedy_index_list, max_num_of_chars)
        ...
        if prt:
            ...
            lead_reward = self.generate_reward(lead_index_list, max_num_of_chars)
            print('Lead3 rewards:', np.array(lead_reward))

        return loss, greedy_reward, greedy_index_list
    ...
    def generate_index_list_and_loss(self, sample_method="sample"):
        ...
        if sample_method == "lead3":
            ...
            return return_summary_index(self.probs_numpy, self.probs_torch, sample_method, self.max_num_of_sents)
